...
def main():
    ...
    ax = fig.add_subplot(
        111, title=TITLE, xlim=(x_f[0], x_f[-1]), ylim=(0, 2 * np.pi * nFFT**2 / RATE)
    )
    ...
    if SAVE:
        frames = int(FPS * SAVE)
        wf = wave.open(
            "D:\\Opinnot\\2023_Oppari\\lighting_controller\\jupyterlab\\frequency_spectrum\\file_example_WAV_1MG.wav",
            "wb",
        )
    ...
    if SAVE:
        ani.save(
            "D:\\Opinnot\\2023_Oppari\\lighting_controller\\jupyterlab\\frequency_spectrum\\temp.gif",
            fps=FPS,
        )
    else:
        plt.show()
